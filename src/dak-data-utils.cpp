...
{
	std::stringstream allSportData(_sportData);

    DAKTSVHeader sportDataLine;

    while (readHeaderLine(allSportData, sportDataLine) == 0) {
        DAKSportData *sportData = new DAKSportData(sportDataLine.sport, sportDataLine.size);
        DAKTSVData fieldDataLine;

        while (readDataLine(allSportData, fieldDataLine) == 0) {
