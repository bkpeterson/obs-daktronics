...
void DAKDataUtils::populateSportsData()
{
	...
	std::stringstream basketballSportData(_sportDataBasketball);

    while (readHeaderLine(basketballSportData, sportDataLine) == 0) {
        DAKSportData *sportData = new DAKSportData(sportDataLine.sport, sportDataLine.size);
        DAKTSVData fieldDataLine;

        while (readDataLine(basketballSportData, fieldDataLine) == 0) {
	...
	std::stringstream footballSportData(_sportDataFootball);

    while (readHeaderLine(footballSportData, sportDataLine) == 0) {
        DAKSportData *sportData = new DAKSportData(sportDataLine.sport, sportDataLine.size);
        DAKTSVData fieldDataLine;

        while (readDataLine(footballSportData, fieldDataLine) == 0) {
            sportData->AddFieldData(fieldDataLine.index, fieldDataLine.field, fieldDataLine.length);
        }
	...
	std::stringstream hockeySportData(_sportDataHockey);

    while (readHeaderLine(hockeySportData, sportDataLine) == 0) {
        DAKSportData *sportData = new DAKSportData(sportDataLine.sport, sportDataLine.size);
        DAKTSVData fieldDataLine;

        while (readDataLine(hockeySportData, fieldDataLine) == 0) {
            sportData->AddFieldData(fieldDataLine.index, fieldDataLine.field, fieldDataLine.length);
        }
	...
	std::stringstream volleyballSportData(_sportDataVolleyball);

    while (readHeaderLine(volleyballSportData, sportDataLine) == 0) {
        DAKSportData *sportData = new DAKSportData(sportDataLine.sport, sportDataLine.size);
        DAKTSVData fieldDataLine;

        while (readDataLine(volleyballSportData, fieldDataLine) == 0) {
            sportData->AddFieldData(fieldDataLine.index, fieldDataLine.field, fieldDataLine.length);
        }
